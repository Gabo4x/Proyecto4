...
def demodulador(senal_Rx, portadora, mpp):
    ...
    # Vector para bits (símbolos) obtenidos por la demodulación
    bits_Rx = np.zeros(N)
...
pixeles = fuente_info('arenal.jpg')
print("Extracción de los pixeles de una imagen (fuente de información)")
print('Dimensiones: ', pixeles.shape, '\n')

bitsRx = rgb_a_bit(pixeles)
print("Codificación de pixeles a una base binaria (bits)")
print(bitsRx)
...
def moduladorQPSK(bits, fc, mpp):
    ...
    portadora_C = np.cos(2*np.pi*fc*t_periodo)
    portadora_S = np.sin(2*np.pi*fc*t_periodo)

    # 3. Inicializar la señal modulada s(t)
    t_simulacion = np.linspace(0, N*Tc, N*mpp) 
    senal_Tx_C = np.zeros(t_simulacion.shape)
    senal_Tx_S = np.zeros(t_simulacion.shape)
    moduladora_C = np.zeros(t_simulacion.shape)  # señal de información
    moduladora_S = np.zeros(t_simulacion.shape)  # señal de información
 
    # 4. Asignar las formas de onda según los bits (BPSK)
    for i, bit in enumerate(bits):
        if bit == 0:
            senal_Tx_C[i*mpp : (i+1)*mpp] = portadora_C
            moduladora_C[i*mpp : (i+1)*mpp] = 1
        else:
            senal_Tx_C[i*mpp : (i+1)*mpp] = portadora_C * -1
            moduladora_C[i*mpp : (i+1)*mpp] = 0
        if bit == 1:
            senal_Tx_S[i*mpp : (i+1)*mpp] = portadora_S
            moduladora_S[i*mpp : (i+1)*mpp] = 1
        else:
            senal_Tx_S[i*mpp : (i+1)*mpp] = portadora_S * -1
            moduladora_S[i*mpp : (i+1)*mpp] = 0
    
    senal_Tx = senal_Tx_C + senal_Tx_S
    
    # 5. Calcular la potencia promedio de la señal modulada
    Pm = (1 / (N*Tc)) * np.trapz(pow(senal_Tx, 2), t_simulacion)
    
    return senal_Tx, Pm, portadora_C, portadora_S, moduladora_C, moduladora_S 
...
def demoduladorQPSK(senal_Rx, portadora_C, portadora_S, mpp):
    '''Un método que simula un bloque demodulador
    de señales, bajo un esquema BPSK. El criterio
    de demodulación se basa en decodificación por 
    detección de energía.
    ...
    # Vector para la señal demodulada
    senal_demodulada_C = np.zeros(M)
    senal_demodulada_S = np.zeros(M)

    # Energía de un período de la portadora
    Es_C = np.sum(portadora_C**2)
    Es_S = np.sum(portadora_S**2)

    # Demodulación
    for i in range(N):
        # Producto interno de dos funciones
        producto_C = senal_Rx[i*mpp : (i+1)*mpp] * portadora_C
        Ep_C = np.sum(producto_C) 
        senal_demodulada_C[i*mpp : (i+1)*mpp] = producto_C
        
        producto_S = senal_Rx[i*mpp : (i+1)*mpp] * portadora_S
        Ep_S = np.sum(producto_S) 
        senal_demodulada_S[i*mpp : (i+1)*mpp] = producto_S
    ...
    return bits_Rx.astype(int), senal_demodulada_C, senal_demodulada_S
...
# 3. Modular la cadena de bits usando el esquema BPSK
senal_Tx, Pm, portadora_C, portadora_S, moduladora_C, moduladora_S = moduladorQPSK(bits_Tx, fc, mpp)

# 4. Se transmite la señal modulada, por un canal ruidoso
senal_Rx = canal_ruidoso(senal_Tx, Pm, SNR)

# 5. Se desmodula la señal recibida del canal
bits_Rx, senal_demodulada_C, senal_demodulada_S = demoduladorQPSK(senal_Rx, portadora_C, portadora_S, mpp)
...
# Visualizar el cambio entre las señales
fig, (ax1, ax2, ax3, ax4, ax5) = plt.subplots(nrows=5, sharex=True, figsize=(14, 7))

# La onda cuadrada moduladora (bits de entrada)
ax1.plot(moduladora_C[0:600], color='y', lw=2) 
ax1.set_ylabel('$b(t)$')
ax2.plot(moduladora_S[0:600], color='r', lw=2) 
ax2.set_ylabel('$b(t)$')
...
# La señal demodulada
ax5.plot(senal_demodulada_C[0:600], color='m', lw=2) 
ax5.set_ylabel('$b^{\prime}(t)$')
...
# Para el punto 4.2 se van a utilizar algunas partes del códgio brindado por el profesor para la realización del laboratorio 4.
...
P = [np.mean(senal_Tx[i]) for i in range(len(t))]
plt.plot(t, P, lw=4, color='g')

# Graficar el resultado teórico del valor esperado
E = np.mean(senal_Tx)*t
plt.plot(t, E, '-.', lw=4, color='r', label='Promedio')
